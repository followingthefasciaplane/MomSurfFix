...
Handle g_hForward_OnBumpIteration;
Handle g_hForward_OnPlayerStuckOnRamp;
Handle g_hForward_OnTryPlayerMovePost;
Handle g_hForward_OnPluginReady;
...
public void OnPluginStart()
{
    ...
    g_hForward_OnBumpIteration = CreateGlobalForward("MomSurfFix_OnBumpIteration", ET_Hook, Param_Cell, Param_Cell, Param_Array, Param_Array);
    g_hForward_OnPlayerStuckOnRamp = CreateGlobalForward("MomSurfFix_OnPlayerStuckOnRamp", ET_Ignore, Param_Cell, Param_Array, Param_Array, Param_Array);
    g_hForward_OnClipVelocity = CreateGlobalForward("MomSurfFix_OnClipVelocity", ET_Hook, Param_Cell, Param_Array, Param_Array, Param_FloatByRef);
    g_hForward_OnTryPlayerMovePost = CreateGlobalForward("MomSurfFix_OnTryPlayerMovePost", ET_Ignore, Param_Cell, Param_Cell, Param_Array, Param_Array, Param_Float);
    g_hForward_OnPluginReady = CreateGlobalForward("MomSurfFix_OnPluginReady", ET_Ignore);

    // Call the OnPluginReady forward
    Call_StartForward(g_hForward_OnPluginReady);
    Call_Finish();
}
...
int TryPlayerMove(CGameMovement pThis, Vector pFirstDest, CGameTrace pFirstTrace)
{
    ...
    for(bumpcount = 0; bumpcount < numbumps; bumpcount++)
    {
        ...
        // Call OnBumpIteration forward
        Action result;
        Call_StartForward(g_hForward_OnBumpIteration);
        Call_PushCell(client);
        Call_PushCell(bumpcount);
        Call_PushArrayEx(original_velocity, 3, SM_PARAM_COPYBACK);
        Call_PushArrayEx(fixed_origin, 3, SM_PARAM_COPYBACK);
        Call_Finish(result);

        if (result == Plugin_Changed)
        {
            vecVelocity.FromArray(original_velocity);
        ...
        if(stuck_on_ramp)
        {
            ...
            // Call OnPlayerStuckOnRamp forward
            Call_StartForward(g_hForward_OnPlayerStuckOnRamp);
            Call_PushCell(client);
            Call_PushArray(original_velocity, 3);
            Call_PushArray(fixed_origin, 3);
            Call_PushArray(valid_plane, 3);
            Call_Finish();
        }
    ...
    // Call OnTryPlayerMovePost forward
    float endVelocity[3], endOrigin[3];
    vecVelocity.ToArray(endVelocity);
    vecAbsOrigin.ToArray(endOrigin);
    Call_StartForward(g_hForward_OnTryPlayerMovePost);
    Call_PushCell(client);
    Call_PushCell(blocked);
    Call_PushArray(endVelocity, 3);
    Call_PushArray(endOrigin, 3);
...
{
    // If we want to add any natives, mark them as optional here. Update include aswell.
}
#endif